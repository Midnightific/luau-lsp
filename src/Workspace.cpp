...
{
    // Delegate to RobloxPlatform when available; otherwise indicate absence
    if (!platform)
        return std::nullopt;

    if (auto* roblox = dynamic_cast<RobloxPlatform*>(platform.get()))
        return roblox->getOvertureLibraryPath(libraryName);
